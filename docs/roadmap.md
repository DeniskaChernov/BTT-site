...
| Квиз на главной | ❌ убран (навигация через каталог) |
...
- Telegram клиенту через коллективный бот (статусы заказа после `/start`)
- Telegram менеджеру — опционально, отдельный chat id (не клиентский бот)

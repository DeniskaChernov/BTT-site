...
export function NavAccountLink({ className }: NavAccountLinkProps) {
  ...
  return (
    <Link
      href="/account"
      className={cn(
        "btt-focus inline-flex h-10 items-center gap-2 rounded-full border border-white/[0.12] bg-white/[0.06] px-3 text-xs font-medium text-stone-200 shadow-none backdrop-blur-xl transition hover:border-white/20 hover:bg-white/[0.09] hover:text-stone-50 motion-reduce:transition-none [box-shadow:inset_0_1px_0_0_rgba(255,255,255,0.1)] md:px-3.5",
        className,
      )}
    >

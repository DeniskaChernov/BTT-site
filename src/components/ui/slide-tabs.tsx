...
import { bttWarmGlassBadgeClass } from "@/lib/ui-classes";
          ...
          {showBadge ? (
            <motion.span
              ...
              className={cn(
                ...
                "pointer-events-none absolute -right-1 -top-1 z-20 flex h-[1.125rem] min-w-[1.125rem] items-center justify-center rounded-full px-1 text-[0.625rem] font-bold leading-none ring-2 ring-[#070605] sm:h-5 sm:min-w-5 sm:text-[0.65rem]",
              )}
              title={badgeHint}
            >
              {badge > 9 ? "9+" : badge}
...
function SlideCursor({ position }: { position: CursorPosition }) {
  ...
  return (
    <motion.li
      ...
      className="pointer-events-none absolute top-1 bottom-1 z-0 rounded-full bg-amber-500/12"
    />
  );
}

...
export default async function AboutPage() {
  ...
  const blocks = [
    { title: t("range"), body: t("range_body") },
        ...
        <AnimatedReveal delay={0.05}>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] px-4 py-3">
              <p className="text-xs uppercase tracking-[0.18em] text-stone-400/80">
                {s("trust_kicker")}
              </p>
        ...
        <div className="mt-10 space-y-5 md:mt-12 md:space-y-6">
          {blocks.map((b, i) => (
            <AnimatedReveal key={b.title} delay={0.06 + i * 0.06}>
              <section className="btt-glass rounded-2xl border-l-2 border-l-white/22 p-6 md:rounded-3xl md:p-8">

...
**Админ-панель (UI)** — в **отдельном проекте CRM** (другой репозиторий). Этот репозиторий (`btt-site`) отдаёт только HTTP API и исходящие webhooks; операторы работают в CRM, которая вызывает admin API с сервера.
